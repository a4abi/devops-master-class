...
- Each kubernetes node can contain multiple pods, whcih can contain mutiple containers. within a pod, all containers can talk to each other using localhost and they all share resources of the host. each of these containers can be from different application of same application
- Namespaces provides isolations for parts of the cluster from other parts of the cluster. ex: dev, QA, create separate namespace for QA and Dev and associate each of the resources with that specific namespace.
